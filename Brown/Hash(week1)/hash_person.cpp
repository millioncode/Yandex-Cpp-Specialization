...
struct AddressHasher {
      ...
      std::size_t operator()(const Address& a) const {
            return (shash(a.city)*k*k+shash(a.street)*k + ihash(a.building) );
      }
};
...
struct PersonHasher {
      ...
      AddressHasher AH;
      const std::size_t k = 113;
      std::size_t operator()(const Person& a) const {
            return (shash(a.name)*k*k+ihash(a.height)*k + dhash(a.weight) + AH(a.address));
      }
};
